...
- ทุก Channel Log รวมถึง Audit และ Log การเงินรวมจะแทรกเส้นคั่นพร้อมวันที่ภาษาไทยก่อนรายการแรกของวันใหม่ตาม timezone ของ Server โดยไม่ส่งซ้ำเมื่อ Bot restart
...
- รูป Airdrop ถูกเก็บแยกใน Channel `รายการเช็กชื่อ` มีลิงก์จากรายชื่อ และตรวจ SHA-256 เพื่อห้ามใช้ไฟล์เดิมซ้ำข้ามรอบ; หัวแก๊ง/รองแก๊งกดปฏิเสธพร้อมระบุเหตุผลได้ ผลจะเปลี่ยนเป็นขาด และสมาชิกส่งรูปใหม่ได้หากรอบยังเปิดอยู่
- สมาชิกแจ้งลาได้ทันทีโดยไม่รออนุมัติ เลือกได้ทั้งคืนหรือเฉพาะตารางกิจกรรมหลายช่วง แก้/ยกเลิกได้ และทุกคนเห็นใบลาใน Channel Log แจ้งลาที่แยกจาก Panel
- ผู้ที่แจ้งลาในรอบนั้นแล้วจะเช็กชื่อทับไม่ได้ แต่ยังเช็กชื่อรอบอื่นได้ ส่วนผู้ที่เช็กชื่อก่อนแล้วแจ้งลา ระบบเปลี่ยนผลจาก `มา` เป็น `ลา` เฉพาะรอบที่เลือก; Admin แก้ผลย้อนหลังพร้อม Audit log
...
2. Dev หรือหัวแก๊งใช้ `/mirubot set-channel` กำหนด Channel ทีละประเภท โดยแยก Channel ใช้งานออกจาก `รายการเช็กชื่อ (หลักฐาน Airdrop)`, `Log ส่งเงินรายสัปดาห์`, `Log เบิกเงินแก๊ง` และ `Log ค่าปรับ`; เมื่อรายการได้รับอนุมัติ รายรับ–รายจ่ายจะถูกบันทึกซ้ำใน `Log การเงินรวม (เงินแก๊ง)` พร้อมยอดคงเหลือล่าสุด
...
Bot จะอัปเดตข้อความสถานะหลักและส่งข้อความแจ้งเตือนใหม่ โดย mention Role Leader, Deputy และ Member ที่กำหนดผ่าน `/mirubot setup-roles` ทุกครั้งที่สถานะเปลี่ยน การเลือกสถานะเดิมซ้ำจะไม่ส่ง notification ซ้ำ เมื่อ deploy ผ่าน Start command `npm start` ระบบจะประกาศ `กำลังอัปเดต` ในขั้น `prestart` และประกาศ `ใช้งานได้ปกติ` หลัง Discord, Database และ health server พร้อมแล้วโดยอัตโนมัติ

## ข่าวอัปเดตบอทอัตโนมัติ

1. สร้างช่องข่าว แล้วให้สมาชิกอ่านได้ และให้ Bot มีสิทธิ์ตามขั้นตอนตั้ง Channel ด้านบน
2. หลัง deploy และรัน migration แล้ว ให้ Dev/หัวแก๊งใช้ `/mirubot set-channel` เลือกประเภท `ข่าวอัปเดตบอท` และช่องที่สร้างไว้
3. Bot จะส่งสรุปเวอร์ชันปัจจุบันให้อัตโนมัติ แม้เพิ่งตั้งช่องหลัง Bot เริ่มทำงานแล้ว

การอัปเดตครั้งถัดไปให้แก้ `src/modules/releases/notes.ts` พร้อมโค้ดของเวอร์ชันนั้น:

- เปลี่ยน `id` เป็นรหัสใหม่ เช่น `2026-09-09.1`; รหัสนี้ใช้ระบุประกาศ ไม่อิง `package.json` หรือ Git commit
- เขียนรายการ `added` (เพิ่ม), `improved` (ปรับ), `fixed` (แก้ปัญหา) เป็นภาษาไทยที่สมาชิกเข้าใจได้ เน้นผลต่อการใช้งาน โดย `added` และ `improved` แสดงรวมใต้หัวข้อ `อัปเดตอะไร` ส่วน `fixed` แสดงใต้ `แก้ไขอะไร` ไม่มีข้อความเกริ่นหรือข้อความปิดท้าย; `title` เก็บเป็นชื่อสรุปภายใน ไม่แสดงในเนื้อหาประกาศ
- หัวข้อที่ไม่มีรายการใช้ `[]` และจะไม่แสดงในประกาศ; ใช้ได้หัวข้อละ 5 รายการ รายการละ 250 ตัวอักษร
- ถ้ารอบนั้นไม่ต้องประกาศ ให้ตั้ง `currentRelease` เป็น `null`
- รัน quality checks แล้ว deploy ตามปกติ Bot จะเข้าคิวประกาศหลัง Discord พร้อมและเปิด health server สำเร็จ ไม่ได้ตรวจยืนยันผล rollout ของ hosting provider

ระบบเก็บเนื้อหาและช่องปลายทางไว้กับคิวถาวร ส่งแยกหนึ่งประกาศต่อรหัสเวอร์ชันต่อ Server โดยไม่ ping สมาชิก การ restart หรือ rollback ไปยังรหัสที่ส่งแล้วจะไม่สร้างประกาศใหม่ การเปลี่ยนช่องไม่ย้ายคิวเดิมหรือส่งประกาศเก่าซ้ำ ควรใช้ Server/ฐานข้อมูลทดสอบแยกจาก production เมื่อรัน `npm run dev` เพราะการเริ่ม Bot ใช้กลไกประกาศเดียวกัน

หาก Discord ขัดข้อง คิวเดิมจะ retry สูงสุด 5 ครั้งตาม scheduler ก่อนเป็น `FAILED` และมี error ใน server log ก่อนส่งทุกครั้ง Bot ตรวจประวัติข้อความของตัวเองเพื่อกู้กรณีส่งสำเร็จแต่ยังบันทึกคิวไม่ทัน พร้อมใช้ nonce เดิมช่วยกันการส่งซ้ำใกล้กัน หากอ่านประวัติไม่ได้หรือเกิน 5,000 ข้อความนับจากท้ายช่อง จะหยุดส่งเพื่อหลีกเลี่ยงประกาศซ้ำ การรับประกัน exactly-once ระหว่าง PostgreSQL กับ Discord ไม่สมบูรณ์ เช่น ข้อความถูกลบก่อน retry

เมื่อคิวเป็น `FAILED` ให้ตรวจและแก้ permission/การเชื่อมต่อ ตรวจว่ามีประกาศในช่องแล้วหรือยัง แล้วให้ผู้ดูแลฐานข้อมูล retry **คิวเดิม** โดยตั้งสถานะ `PENDING`, `attempts = 0`, `run_at = now()`, `locked_at = null`, `locked_by = null` เฉพาะแถวของ Server และ `deduplication_key` นั้น ห้ามเปลี่ยนรหัสเวอร์ชันเพื่อแก้ปัญหาการส่ง เพราะอาจทำให้ประกาศซ้ำ
...
- Start command: `npm start` (ต้องใช้คำสั่งนี้เพื่อให้ lifecycle สถานะ Bot ทำงานอัตโนมัติ)
- Health path: `/health`
